...
int test() {
    ...
    while (!is_end(graph)) {
        ...
        if (action >= graph.num_nodes) {
            ordering.push_back(action - graph.num_nodes);
        } else {
    ...
    bool valid = true;
    std::sort(sortedOrdering.begin(), sortedOrdering.end(), 
        [](const int& a, const int& b) 
            {
                return a < b;
            });
    for (unsigned int i = 0; i < sortedOrdering.size(); i++) {
        if ((unsigned int) sortedOrdering[i] != i) {
            valid = false;
            break;
        }
    }
    ...
    for (unsigned int i = 0; i < (unsigned int) graph.num_nodes; i++) {
        if (adj_black[i]) {
            blacks.push_back(i);
        } else if (adj_white[i]) {
            whites.push_back(i);
